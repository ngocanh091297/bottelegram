...
async function getInfor2(token, refreshToken, username) {
    try {
        let body = {
            language: 2,
            random: It(),
...
async function test(bot) {
    ...
    try {
        ...
        let data = await axios.post("https://82vn82vnapi.com/api/webapi/GetTRXGameIssue", body, {
            headers: {
                'content-type': 'application/json;charset=UTF-8',
        ...
        if (data.data && data.data.code == 0) {
            let datalist = data.data.data.predraw
...
async function check_dk(issuenumber, bot) {
    ...
    try {
        ...
        let list_lich_su = await axios.post("https://82vn82vnapi.com/api/webapi/GetTRXNoaverageEmerdList", body, {
            headers: {
                'content-type': 'application/json;charset=UTF-8',
                'Authorization': `Bearer ${token}`,
                'User-Agent': userAgent
        ...
        if (list_lich_su.data && list_lich_su.data.data && list_lich_su.data.code == 0) {
            let gameslist = list_lich_su.data.data.data.gameslist;
            //  ["3L_N","3N_L"]
...
async function dangnhaplaitk(user, bot, chatId) {
    ...
    if (newtoken && newtoken.data && newtoken.data.data && newtoken.data.data.token) {
        ...
        if (user3 && user3.data && user3.data.code == 0 && user3.data.data.token) {
            ...
            await db(table).update('trxhash1', 0).where('id', user.id)
...
async function vaolenhtaikhoan(item, element, issuenumber, bot) {
    ...
    try {
        ...
        if (body.betCount >= 1) {
            result = await axios.post("https://82vn82vnapi.com/api/webapi/GameTRXBetting", body, {
                headers: {
                    'content-type': 'application/json;charset=UTF-8',
        ...
        if (result.data) {
            ...
            if (result.data && result.data.msg == 'Bet success' && result.data.code == 0) {
                ...
                bot.sendMessage(item.tele_id, `✅ Đã đặt cược TRX Hash 1 ${body.selectType == 13 ? "Lớn" : "Nhỏ"} - ${body.betCount}000đ - Kỳ xổ ${issuenumber}`,)
            } else {
                //  đặt cược lỗi
