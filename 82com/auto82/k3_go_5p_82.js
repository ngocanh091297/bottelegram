...
async function test(bot) {
    ...
    try {
        ...
        let data = await axios.post("https://82vn82vnapi.com/api/webapi/GetGameK3Issue", body, {
            headers: {
                'content-type': 'application/json;charset=UTF-8',
...
async function check_dk(issuenumber, bot) {
    ...
    try {
        ...
        let list_lich_su = await axios.post("https://82vn82vnapi.com/api/webapi/GetK3NoaverageEmerdList", body, {
            headers: {
                'content-type': 'application/json;charset=UTF-8',
                'Authorization': `Bearer ${token}`,
                'User-Agent': userAgent
...
async function dangnhaplaitk(user, bot, chatId) {
    ...
    if (newtoken && newtoken.data && newtoken.data.data && newtoken.data.data.token) {
        ...
        if (user3 && user3.data && user3.data.code == 0 && user3.data.data.token) {
            ...
            await db(table).update('k3go5', 0).where('id', user.id)
    ...
    try {
        ...
        if (result.data) {
            ...
            if (result.data && result.data.msg == 'Bet success' && result.data.code == 0) {
                ...
                bot.sendMessage(item.tele_id, `✅ Đã đặt cược K3-GO 5 ${body.selectType == "C" ? "Chẵn" : "Lẻ"} - ${body.betCount}000đ - Kỳ xổ ${issuenumber}`,)
            } else {
                //  đặt cược lỗi
